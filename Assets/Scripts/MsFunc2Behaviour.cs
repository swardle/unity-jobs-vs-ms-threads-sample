...
//https://www.jacksondunstan.com/articles/5487
...
public class MsFunc2Behaviour : MonoBehaviour
{
    ...
    class DropPoolStart : MyTask
    {
        ...
        public MyVoidPointer<DropPoolJob> m_dropPoolJobPtr;
        public DropPoolJob m_dropPoolJob;

        private delegate void ExecuteStaticDelegate(IntPtr param);
        private FunctionPointer<ExecuteStaticDelegate> m_FunctionPointer;
        private ExecuteStaticDelegate m_FunctionPointerInvoke;

        public DropPoolStart()
        {
            m_dropPoolJob.Init();
            m_dropPoolJobPtr = new MyVoidPointer<DropPoolJob>(ref m_dropPoolJob);

            m_timer = new Stopwatch();

            m_FunctionPointer = BurstCompiler.CompileFunctionPointer<ExecuteStaticDelegate>(DropPoolJob.ExecuteStatic);
            m_FunctionPointerInvoke = m_FunctionPointer.Invoke;
        }
        public void Dispose()
        {
            m_dropPoolJob.Dispose();
            m_dropPoolJobPtr.Dispose();
        }

        public void Execute()
        {
            m_timer.Restart();
            m_FunctionPointerInvoke(m_dropPoolJobPtr.m_ptr);
            m_timer.Stop();
        }
    ...
    struct DropPoolJob
    {
        ...
        {
            ref DropPoolJob dropPool = ref UnsafeUtility.ArrayElementAsRef<DropPoolJob>(ptr.ToPointer(), 0);
            dropPool.RunJob();
        }
    ...
    public void Awake()
    {
        ...
        for (int i = 0; i < m_rawImages.Length; i++)
        {
            ...
            ref DropPoolJob dropPoolJob = ref m_dropPoolStarts[i].m_dropPoolJob;
    ...
    public void Update()
    {
        ...
        m_timerText.text = $"Ms Func = {updateTime.TotalMilliseconds:0.##}";
    }
    ...
    private Vector2 TryToClickPool(PointerEventData eventData, RawImage rawImage, int image)
    {
        ...
        if (inImage)
        {
            ...
            ref DropPoolJob dropPoolJob = ref m_dropPoolStarts[image].m_dropPoolJob;
