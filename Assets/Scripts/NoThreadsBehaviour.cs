...
public class NoThreadsBehaviour : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (m_enableSystem.isOn)
        {
            ...
            for (int i = 0; i < m_rawImages.Length; i++)
            {
                DropPool dropPool = m_dropPools[i];
                dropPool.UpdatePool();
                updateTime += TimeSpan.FromTicks(dropPool.m_timer.ElapsedTicks);
            }
        }

        m_timerText.text = $"No Thread = {updateTime.TotalMilliseconds:0.##}";
    }
